...
const Works = () => {
  return (
    <section id="works" className={`${layout.section}`}>
      ...
      <div
        className={`grid place-items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5`}
      >
        {works.map((website) => (
          <div key={website.alt} className="max-w-[380px] mx-auto relative">
            ...
            <div className="overlay">
              <a href={website.link1} className="overlay-icon">
                <FiGithub />
              </a>
              <a href={website.link2} className="overlay-icon">
                <FiEye />
              </a>
            </div>
          </div>
